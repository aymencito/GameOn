// GameOn 2.5 expiry + organiser request UX patch
// 1) Past games are excluded from active player/organiser flows.
// 2) Empty organiser request lists show a normal empty state, not a misleading counter warning.

(function(){
  const originalRenderOpenGames = typeof renderOpenGames === 'function' ? renderOpenGames : null;
  if (originalRenderOpenGames) {
    renderOpenGames = function(){
      const today = localISO();
      openCache = (openCache || []).filter(g => !g.game_date || g.game_date >= today);
      return originalRenderOpenGames();
    };
  }

  const originalOpenRequests = typeof openRequests === 'function' ? openRequests : null;
  if (originalOpenRequests) {
